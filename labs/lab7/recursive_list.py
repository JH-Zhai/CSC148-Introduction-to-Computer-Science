...
class RecursiveList:
    ...
    def selections(self) -> List[RecursiveList]:
        ...
        if self.is_empty():
            ...
            re_val = []
            for rest_selection in rest_selections:
                re_val.append(rest_selection)
                new = RecursiveList([self._first])
                new._rest = rest_selection
                re_val.append(new)
            return re_val
    ...
    def permutations(self) -> List[RecursiveList]:
        """
        >>> lst1 = RecursiveList([])
        >>> permutations1 = lst1.permutations()
        >>> len(permutations1)
        1
        >>> permutations1[0].is_empty()
        True
        >>> lst2 = RecursiveList([1, 2, 3])
        >>> len(lst2.permutations())
        6
        """
        if self.is_empty():
            return [RecursiveList([])]
            ...
            for i in range(len(self)):
                # 拿出 index i
                taken = self.pop(i)
                rest_permutations = self.permutations()
                for rest_permutation in rest_permutations:
                    new = RecursiveList([taken])
                    new._rest = rest_permutation
                    re_var.append(new)
                # 放回 index i
                self.insert(i, taken)
            return re_var
