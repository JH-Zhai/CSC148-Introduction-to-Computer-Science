...
class BinarySearchTree:
    ...
    def count(self, item: Any) -> int:
        ...
        if self._root == item:
            re_var += 1
            re_var += self._left.count(item)
            re_var += self._right.count(item)
            return re_var
        else:
            if item < self._root:
                re_var += self._left.count(item)
    ...
    def items(self) -> List:
        ...
        if self.is_empty():
            ...
            re_var.extend(self._left.items())
            re_var.append(self._root)
            re_var.extend(self._right.items())
            return re_var
    ...
    def smaller(self, item: Any) -> List:
        ...
        if self.is_empty():
            ...
            if item <= self._root:
                re_var.extend(self._left.smaller(item))
            else:
                re_var.extend(self._left.smaller(item))
                re_var.append(self._root)
                re_var.extend(self._right.smaller(item))
            return re_var
